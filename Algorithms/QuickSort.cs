...
namespace SortViewer.Algorithms
{
    public class QuickSort : BaseSortAlgorithm
    {
        ...
        public override List<SortingStep> Sort(int[] data)
        {
            ...
            _steps.Add(CreateStep(array, OperationType.Initial, Array.Empty<int>(), Array.Empty<int>(), "Initial state"));
        ...
        private int Partition(int[] array, int low, int high)
        {
            ...
            if (array[mid] < array[low]) {
                Swap(array, low, mid);
                _steps.Add(CreateStep(array, OperationType.Swap, Array.Empty<int>(), new int[] { low, mid }, $"Swapping {array[mid]} and {array[low]}"));
                RaiseSwapEvent(low, mid, array);
            }
                
            if (array[high] < array[low]) {
                Swap(array, low, high);
                _steps.Add(CreateStep(array, OperationType.Swap, Array.Empty<int>(), new int[] { low, high }, $"Swapping {array[high]} and {array[low]}"));
                RaiseSwapEvent(low, high, array);
            }
                
            if (array[mid] < array[high]) {
                Swap(array, mid, high);
                _steps.Add(CreateStep(array, OperationType.Swap, Array.Empty<int>(), new int[] { mid, high }, $"Swapping {array[high]} and {array[mid]}"));
            ...
            _steps.Add(CreateStep(array, OperationType.Read, new int[] { high }, Array.Empty<int>(), $"Choosing pivot: {pivot}"));
            ...
            for (int j = low; j < high; j++)
            {
                ...
                _steps.Add(CreateStep(array, OperationType.Comparison, new int[] { j, high }, Array.Empty<int>(), $"Comparing {array[j]} with pivot {pivot}"));
                ...
                if (comparisonResult <= 0)
                {
                    ...
                    if (i != j)
                    {
                        ...
                        _steps.Add(CreateStep(array, OperationType.Swap, Array.Empty<int>(), new int[] { i, j }, $"Swapping {array[i]} and {array[j]}"));
                    }
                }
            }
            ...
            if (i + 1 != high)
            {
                ...
                _steps.Add(CreateStep(array, OperationType.Swap, Array.Empty<int>(), new int[] { i + 1, high }, $"Placing pivot {pivot} in position {i + 1}"));
            }
